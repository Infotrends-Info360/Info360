...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ,'<div class="layui-layim-info">'
      ,'<div class="layui-layim-user" id="userNickName">Alex</div>'
    ...
    ,'{{# } }}">'
      ...
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="聯絡人" layim-event="tab" lay-type="friend">&#xe612;</li>'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isgroup){ }}'
      ,' layim-hide'
      ...
      /**
      ,'<li class="layui-icon" title="歷史對話" layim-event="tab" lay-type="history">&#xe611;</li>'
      */
    ,'</ul>'
    ...
    ,'<div class="layim-chat-footer">'
      ...
      ,'<div class="layim-chat-bottom">'
        ,'<div class="layim-chat-send">'
          ,'{{# if(!d.base.brief){ }}'
          ,'<span class="layim-send-close" layim-event="closeThisChat" onclick="showCloseDialog()">關閉</span>'
          ,'{{# } }}'
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>'
    ,'<div class="layim-chat-user"><img src="resources/layui/css/pc/layim/skin/new_logo.jpg"><cite>'
    ,'{{# if(d.mine){ }}'
      ,'<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"Agent" }}'
     ,'{{# } else { }}'
      ,'{{ d.username||"Agent" }}<i>{{ layui.data.date(d.timestamp) }}</i>'
     ,'{{# } }}'
      ,'</cite></div>'
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/new_logo.jpg')) +'"><span>'+ (content||cache.base.title||'線上即時通') +'</span>'
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(chat[0]){
      ...
      if(list.find('li').length === 1 && !listThat[0]){
        hasFull || layimChat.css('width', 650);
        list.css({
  ...
  var sendMessage = function(){
    var data = {
      ...
      ,avatar: cache.mine ? cache.mine.avatar : (layui.cache.dir+'css/pc/layim/skin/new_logo.jpg')
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || '客戶') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
